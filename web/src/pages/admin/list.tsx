                  ...
                  <tr key={u.id} className="border-b-2 border-ink hover:bg-neutral-light transition-colors">
                    ...
                    <td className="p-4">
                      <span
                        className={`px-3 py-1 border-2 border-ink font-bold text-sm ${
                          u.approved ? 'bg-primary text-ink' : 'bg-secondary text-ink'
                        }`}
                      >
                    ...
                    <td className="p-4">
                      <div className="flex gap-2">
                        {!u.approved ? (
                          <>
                            <Button variant="primary" size="sm" onClick={() => approveMutation.mutate(u.id)} disabled={isProcessing}>
                              Approve
                            </Button>
                            <Button variant="secondary" size="sm" onClick={() => rejectMutation.mutate(u.id)} disabled={isProcessing}>
                              Reject
                            </Button>
                          </>
                        ) : (
                          <Button variant="outline" size="sm" onClick={() => disableMutation.mutate(u.id)} disabled={isProcessing}>
                            Disable
                          </Button>
                        )}
                      </div>
...
export default function AdminListPage() {
  return (
    <>
      <Head>
        <title>User List · trunk-xyz</title>
      </Head>
